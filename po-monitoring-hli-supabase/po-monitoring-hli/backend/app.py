...
def import_source_column_map(df, columns):
    ...
    # For 'so': always anchor to the known column letter (AK for RM, AM for SP)
    # when sheet kind can be determined. This prevents a misplaced SO header
    # (e.g. in a section-label row) from mapping 'so' to the wrong column.
    ...
    if _so_col_fallback and _so_col_kind in ('rm', 'sp'):
        try:
            source_map['so'] = column_index_from_letter(_so_col_fallback) - 1
        except Exception:
            pass
    return source_map
...
def import_source_rows_fast(source, columns, vendor_set):
    ...
    try:
        ...
        # For 'so' specifically: always use the positional fallback (AK for RM,
        # AM for SP) when the sheet kind is known. Header detection can pick up
        # a wrong 'SO' column if the sheet has duplicate or misplaced SO headers
        # (e.g. a section-label row that happens to contain the word SO),
        # while the column letter is a reliable anchor for both source formats.
